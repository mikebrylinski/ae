...
const SORTS: GallerySort[] = ['newest', 'oldest', 'tag']
...
export function GalleryGridHeader() {
  ...
  return (
    <div className="glass-card glass-card--aurora p-6 sm:p-8 md:p-10">
      ...
      >
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-10 md:items-start">
          <div
            className="space-y-6"
            role="group"
            aria-label={t.a11y.filterGallery}
          >
...
export function GalleryGrid({ children }: { children?: ReactNode }) {
  ...
  const items = useMemo(() => filterGallery(selected, sort), [selected, sort])
...
function GalleryTile({
  ...
  return (
    <li
      className="mb-4 break-inside-avoid [content-visibility:auto] [contain-intrinsic-size:280px]"
    >
      <button
        type="button"
      ...
      >
        <MediaImage
          ...
          wrapperClassName="isolate overflow-hidden rounded-[1rem] border border-border transition-[border-color,box-shadow] duration-500 group-hover:border-primary/40 group-hover:shadow-[0_0_24px_rgba(184,255,0,0.06)]"
